...
export default function AutocorrectDashboard() {
    const [runs] = useState(MOCK_RUNS);
    const [selected, setSelected] = useState<any>(null);

    // Safe localStorage init for Next.js SSR
    const [log, setLog] = useState<any[]>([]);
    useEffect(() => {
        try { setLog(JSON.parse(localStorage.getItem("aitdl_log") || "[]")); } catch { setLog([]); }
    }, []);
    ...
    const handleApprove = (runId: string, issueId: string) => {
    ...
    const handleReject = (runId: string, issueId: string) => {
    ...
    const handleApproveAll = (runId: string) => {
        const run = runs.find(r => r.id === runId);
                            ...
                            <div key={run.id}
                                ...
                                <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                                    ...
                                    <div style={{ display: "flex", gap: 8, alignItems: "center", flexShrink: 0 }}>
                                        <Badge label={run.conclusion.toUpperCase()} color={CONCL_COLOR[run.conclusion as keyof typeof CONCL_COLOR] || "#888"} />
                                        {run.issues.length > 0 && (
                                ...
                                {selected?.id === run.id && run.issues.length > 0 && (
                                    <div style={{ marginTop: 16 }}>
                                        ...
                                        {run.issues.map(issue => {
                                            ...
                                            return (
                                                <div key={issue.id} className="issue-card" style={{ background: SEV_BG[issue.severity as keyof typeof SEV_BG] || "#11141d" }}
                                                    onClick={e => e.stopPropagation()}>
                                                    <div style={{ display: "flex", alignItems: "flex-start", gap: 12 }}>
                                                        <div style={{ flex: 1 }}>
                                                            <div style={{ display: "flex", gap: 8, marginBottom: 6, flexWrap: "wrap" }}>
                                                                <Badge label={issue.severity.toUpperCase()} color={SEV_COLOR[issue.severity as keyof typeof SEV_COLOR]} />
                                ...
                                run.issues.length > 0 && (
                                    <div key={run.id} style={{ marginBottom: 28 }}>
                                        ...
                                        {run.issues.map(issue => {
                                            const state = approvals[`${run.id}_${issue.id}`];
                                            return (
                                                <div key={issue.id} className="issue-card" style={{ background: SEV_BG[issue.severity as keyof typeof SEV_BG] }}>
                                                    <div style={{ display: "flex", alignItems: "flex-start", gap: 12 }}>
                                                        <div style={{ flex: 1 }}>
                                                            <div style={{ display: "flex", gap: 8, marginBottom: 6, flexWrap: "wrap" }}>
                                                                <Badge label={issue.severity.toUpperCase()} color={SEV_COLOR[issue.severity as keyof typeof SEV_COLOR]} />
