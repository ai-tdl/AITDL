...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return {
    title: dict.ecom.seo.title,
    description: dict.ecom.seo.description,
  }
}
...
export default async function EcomPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params
  const dict = await getDictionary(locale)
