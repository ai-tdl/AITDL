...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return {
    title: dict.teacher.seo.title,
    description: dict.teacher.seo.description,
  }
}
      ...
      <section className="relative px-6 pt-20 pb-28 overflow-hidden">
        ...
        <div className="relative max-w-5xl mx-auto text-center">
          ...
          <h1 className="font-heading text-6xl md:text-8xl font-extrabold text-white mb-6 leading-none tracking-tight uppercase">
            {dict.teacher.hero.title}
          </h1>

          <p className="text-xl text-zinc-300 max-w-2xl mx-auto mb-4">
            {dict.teacher.hero.subtitle}
          </p>
          
          <div className="flex flex-col sm:flex-row gap-4 justify-center mt-12">
            <a href="#contact" className="btn-premium btn-premium-emerald px-10">Start Teaching Smart →</a>
            <a href={`/${locale}/explore`} className="px-8 py-3.5 rounded-xl border border-white/10 hover:border-white/25 text-white font-medium transition-colors text-sm">Explore Tools</a>
          </div>
        </div>
      </section>
